...
def generate_summary_report(
    ...
    from dshelper import eda, overview, stats
...
def _generate_html_report(df, title, include_plots):
    ...
    from dshelper import eda, overview

    # Get basic statistics
    summary = overview.quick_look(
        df, name=title, include_describe=True, include_info=False
    )
...
def export_to_excel(df, filename, include_summary=True, include_plots=False):
    ...
    try:
        ...
        print(
            "Error: This function requires openpyxl. Install it with 'pip install openpyxl'."
        )
        return
...
def generate_data_dictionary(df, filename=None, include_stats=True):
    ...
    for col in df.columns:
        ...
        if include_stats:
            ...
            data_dict["Example Values"].append(
                ", ".join(str(val) for val in example_values)
            )
