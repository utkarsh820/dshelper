...
def to_numeric_cols(df, columns):
    ...
    for col in columns:
        if col in df.columns:
            ...
            if nan_count > 0:
                print(
                    f"Column '{col}': Converted {converted_count} values to numeric. {nan_count} values could not be converted and were set to NaN."
                )
            else:
...
def to_datetime_cols(df, cols=None):
    ...
    for col in cols:
        if col in df.columns:
            ...
            if nan_count > 0:
                print(
                    f"Column '{col}': Converted {converted_count} values to datetime. {nan_count} values could not be converted and were set to NaN."
                )
            else:
...
def to_category_cols(df, cols=None, min_unique=10, max_unique_ratio=0.1):
    ...
    for col in cols:
        if col in df.columns:
            ...
            print(
                f"Column '{col}': Converted to category type with {result[col].cat.categories.size} categories."
            )
        else:
            print(f"Column '{col}' not found in DataFrame.")
...
def memory_usage(df, deep=True):
    ...
    results = {
        ...
        "savings_percentage": (
            (1 - optimized_usage / current_usage) * 100 if current_usage > 0 else 0
        ),
    ...
    print(
        f"Memory savings: {results['savings'] / 1e6:.2f} MB ({results['savings_percentage']:.2f}%)"
    )
    ...
    for col in df.columns:
        if str(df[col].dtype) != str(optimized_df[col].dtype):
            print(
                f"- Convert '{col}' from {df[col].dtype} to {optimized_df[col].dtype}"
            )
